...
const controller: Controller[] = [
  ...
  // 检查文件
  {
    ...
    handler: [
      async (ctx) => {
        const { fileKey, fileSuffix } = ctx.request.body;
  ...
  // 上传分片
  {
    ...
    handler: [
      ...
      async (ctx) => {
        ...
        fs.rename(file.filepath, path.join(__dirname, `../upload/${fileKey}/${shardIndex}`));
  ...
  // 合并分片
  {
    ...
    handler: [
      async (ctx) => {
        ...
        const shardList = await fs.readdir(path.join(__dirname, `../upload/${fileKey}`));
        if (shardList.length !== shardTotalCount || shardList.length === 0) {
          ctx.body = AjaxResult.success('切片数量不符！');
        } else {
          const filePath = path.join(__dirname, `../upload/${fileKey}.${fileSuffix}`);
          await fs.writeFile(filePath, '');
          for (let i = 0; i < shardList.length; i++) {
            const shardPath = path.join(__dirname, `../upload/${fileKey}/${i}`);
            const unit8Array = new Uint8Array(await fs.readFile(shardPath));
            await fs.appendFile(filePath, unit8Array);
